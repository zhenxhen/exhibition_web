...
// 6. Handle resizing (브라우저 사이즈에 실시간 반응하는 반응형 카메라)
function handleResize() {
  ...
  // 사용자가 설정한 기본 카메라 거리 300을 가이드라인으로 유지하되,
  // 좁은 화면(모바일 등)에서는 잘림을 방지하기 위해 카메라를 뒤로 뺍니다.
  const BASE_Z = 350;
  const BASE_ASPECT = 16 / 9; // 일반적인 데스크톱 와이드 모니터 비율 기준
...
// 최초 로딩 시에도 즉각 현재 화면 비율에 맞춰 카메라 거리 자동 조절 실행
handleResize();
